...
const char *LogInterfaceString[STLINK_NB_INTERFACES] = {"DBG", "DBG2", "DBG SERVER", "BRIDGE"};
 ...
 */
StlinkDevice::StlinkDevice(STLinkInterface &StlinkIf) : m_bStlinkConnected(false), m_bOpenExclusive(false), m_pStlinkInterface(&StlinkIf) {
    m_handle = NULL;
...
/*
 * @brief Opening routine based on the serial number of the STLink; to use in case StlinkInstId is
 * no more reliable (devices have been reenumerated between STLink instance selection and opening)
 * or to choose a specific STLink by its serial number.
 *
 ...
 * @retval #STLINKIF_NO_ERR If no error
 */
STLinkIf_StatusT StlinkDevice::SendRequest(STLink_DeviceRequestT *pDevReq, const uint16_t UsbTimeoutMs) {
    STLinkIf_StatusT ifStatus;
