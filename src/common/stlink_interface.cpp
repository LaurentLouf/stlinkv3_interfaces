...
const char *LogIfString[STLINK_NB_INTERFACES] = {"DBG", "DBG2", "DBG SERVER", "BRIDGE"};
...
STLinkInterface::STLinkInterface(STLink_EnumStlinkInterfaceT IfId)
    : m_ifId(IfId), m_nbEnumDevices(0), m_bApiDllLoaded(false), m_bDevInterfaceEnumerated(false) {
#ifdef WIN32  // Defined for applications for Win32 and Win64.
    m_hMod = NULL;
#endif
    m_pathOfProcess[0] = '\0';
...
/*
 * Trace logging mechanism, under compilation switch USING_ERRORLOG (requiring ErrLog.h and
 * ErrLog.cpp)
 */
...
STLinkIf_StatusT STLinkInterface::LoadStlinkLibrary(const char *pPathOfProcess) {
    ...
    if (m_bApiDllLoaded == false) {
...
#endif
                // Note: Unicode is not supported (would require T_CHAR szDllPath, to include
                // <tchar.h> and to use generic function PathAppend(szDllPath,
                // _T("STLinkUSBDriver.dll"))
        ...
        if (m_hMod == NULL) {
            // Second try using the whole procedure for path resolution (including PATH environment
            // variable)
        ...
        if (ifStatus == STLINKIF_NO_ERR) {
            ...
            if (m_ifId == STLINK_BRIDGE) {
                ...
                // Check if DLL supports at least required function, this is mandatory but not
                // enough for BRIDGE support STLink_Reenumerate will return SS_BAD_PARAMETER if DLL
                // is too old and Bridge interface is not supported.
                if ((STLink_Reenumerate == NULL) || (STLink_GetNbDevices == NULL) || (STLink_GetDeviceInfo2 == NULL) ||
                    (STLink_OpenDevice == NULL) || (STLink_CloseDevice == NULL) || (STLink_SendCommand == NULL)) {
 ...
 * @param[out] pNumDevices Pointer where the function returns the number of connected STLink m_ifId
 * interfaces. Can be NULL if not wanted.
 *
 * @retval #STLINKIF_NOT_SUPPORTED If STLinkUSBDriver is too old for given m_ifId or m_ifId not
 * supported yet
...
STLinkIf_StatusT STLinkInterface::EnumDevices(uint32_t *pNumDevices, bool bClearList) {
    ...
    if (IsLibraryLoaded() == true) {
        if (m_ifId == STLINK_BRIDGE) {
            ...
            // Note that STLink_Reenumerate might fail because of issue during serial number
            // retrieving which is not a blocking error here;
 ...
 * (or refresh device list after change in the system).
 *
 * @param[in]  bForceRenum Set to true in order to force the renumeration (refresh device list after
 * change in the system). Otherwise the list remains as it was during the previous call.
 * @param[in]  bClearList  If true:\n
 *               - All opened devices are closed and handles are deleted.\n
 *               - The use of a previously returned handle is UNPREDICTABLE (might either fail
 *               because the handle is no more known, or might address a newly renumerated
 ...
 * @warning #STLINKIF_PERMISSION_ERR can be returned if one device is already opened by another
 * program, however the device is listed in pNumDevices.
 *
 * @retval #STLINKIF_NOT_SUPPORTED Parameter(s) error
 * @retval #STLINKIF_DLL_ERR Error in loading STLinkUSBDriver library or too old not supporting
 * current Interface
 ...
 *               is greater (caller more recent than STLinkUSBDriver library), exceeding fields are
 * set to 0.
 ...
 * will return #STLINKIF_STLINK_SN_NOT_FOUND if the given SerialNumber is not found.\n If bStrict is
 ...
 * @retval #STLINKIF_NO_ERR If no error
 */
STLinkIf_StatusT STLinkInterface::OpenDevice(const char *pSerialNumber, bool bStrict, uint32_t StlinkIdTcp, bool bOpenExclusive,
                                             void **pHandle) {
    STLinkIf_StatusT ifStatus = STLINKIF_NO_ERR;
    int stlinkInstId;
    ...
    if ((bStrict == false) && (m_nbEnumDevices == 1)) {
        // There is currently only one device connected, and the caller did not expected a full
        // matching
        LogTrace("STLink serial number (%s) not found; opening the (lonely) connected STLink (SN=%s)", pSerialNumber, pEnumUniqueId);
 ...
 * @retval #STLINKIF_NO_ERR If no error
 */
STLinkIf_StatusT STLinkInterface::SendCommand(void *pHandle, uint32_t StlinkIdTcp, STLink_DeviceRequestT *pDevReq,
    ...
    if (IsLibraryLoaded() == true) {
        if (m_ifId == STLINK_BRIDGE) {
            ...
            if (ret != SS_OK) {
                LogTrace(
                    "%s USB communication error (%d) after target cmd %02hX %02hX %02hX %02hX "
                    "%02hX %02hX %02hX %02hX %02hX %02hX",
                    LogIfString[m_ifId], (int)ret, (unsigned short)pDevReq->CDBByte[0], (unsigned short)pDevReq->CDBByte[1],
