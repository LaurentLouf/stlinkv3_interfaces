...
// Constants defining the type of info to retrieve (STMass_GetDeviceInfo) - Old API required for
// ST-Link/V1
...
#define REQUEST_READ_EP3 \
    3  // Through EP3IN or EP2IN for DBG/TCP interface only: Compatibility break if that value was
       // used for reading through EP1
// All other values are for read through EP1IN, but a constant is provided anyway for upper layers
// lisibility
...
#define REQUEST_READ_DATA_2ND_EPIN \
    REQUEST_READ_EP3  // read req (data only) through 2nd bulk IN EP of the given interface (if it
                      // exists)
...
typedef struct {
    ...
    uint8_t InputRequest;                  // 0 (output) or input on ST-Link/V1, REQUEST_WRITE, REQUEST_READ_EP1 or
                                           // REQUEST_READ_EP3 on ST-Link/V2
...
} TDeviceRequest, *PDeviceRequest;         // TDeviceRequest, *PDeviceRequest: legacy library name, use
                                           // STLink_DeviceRequestT

typedef TDeviceRequest STLink_DeviceRequestT;

// Note keep TDeviceRequest before STLink_DeviceRequestT because old library only know
// TDeviceRequest
///////////////////////////////////////////////////////////////////////////////
...
typedef struct {
    char DevPath[MAX_PATH];                     // Device pathname returned by SetupDiGetDeviceInterfaceDetail on
                                                // ST-Link/V2 Or disk name like "\\.\A:" on ST-Link/V1
    char EnumUniqueId[SERIAL_NUM_STR_MAX_LEN];  // Unique instance ID from system enumeration (equal
                                                // to device Serial number in some cases, but not
                                                // always ...)
...
typedef struct {
    uint32_t StLinkUsbId;                       ///< ST-LINK-SERVER Device cookie in little endian format, to use in
                                                ///< STLINK_TCP_CMD_OPEN_DEVICE
    char EnumUniqueId[SERIAL_NUM_STR_MAX_LEN];  ///< Unique instance ID from system enumeration
                                                ///< (equal to device Serial number in some cases,
                                                ///< but not always ...)
    uint16_t VendorId;                          ///< Vendor  ID from USB device descriptor (system enumeration)
    uint16_t ProductId;                         ///< Product ID from USB device descriptor (system enumeration)
    uint8_t DeviceUsed;                         ///< On windows, equal to 1 if device interface was already opened from
...
typedef enum {
    STLINK_DBG_INTERFACE = 0,  // Note that the value is also the interface index in the device
                               // (important for implementation on Linux)
    STLINK_RW_INTERFACE,       // Note that the value is also the interface index in the device
                               // (important for implementation on Linux)
...
#define STLINK_USB_BRIDGE_IF_NB \
    4  // Bridge interface index in the device (important for implementation on Linux) when mass
       // storage interface is present
#define STLINK_USB_BRIDGE_IF_NB_NO_MSD \
    3  // Bridge interface index in the device (important for implementation on Linux) when no mass
       // storage interface
...
// Returned: SS_OK  = SUCCESS, Error otherwise (Error Code)
//******************************************************************************
typedef uint32_t(STD_CALL *pSTMass_GetDeviceInfo)(PDevice pDevice, uint32_t Kind, char *pBuffer, uint32_t BufferLength);
...
// Returned: SS_OK  = SUCCESS, Error otherwise (Error Code)
//******************************************************************************
typedef uint32_t(STD_CALL *pSTMass_SendCommand)(PDevice pDevice, void *pHandle, PDeviceRequest pRequest, uint32_t DwTimeOut);
...
#ifdef WIN32
// Export the signature only; the routine will be linked through GetProcAdddress
typedef uint32_t(STD_CALL *pSTLink_GetDeviceInfo)(TEnumStlinkInterface IfId, uint8_t DevIdxInList, TDeviceInfo *pInfo, uint32_t InfoSize);
#else
EXPORTED_API uint32_t STD_CALL STLink_GetDeviceInfo(TEnumStlinkInterface IfId, uint8_t DevIdxInList, TDeviceInfo *pInfo, uint32_t InfoSize);
#endif
...
#ifdef WIN32
// Export the signature only; the routine will be linked through GetProcAdddress
typedef uint32_t(STD_CALL *pSTLink_GetDeviceInfo2)(TEnumStlinkInterface IfId, uint8_t DevIdxInList, TDeviceInfo2 *pInfo, uint32_t InfoSize);
#else
EXPORTED_API uint32_t STD_CALL STLink_GetDeviceInfo2(TEnumStlinkInterface IfId, uint8_t DevIdxInList, TDeviceInfo2 *pInfo,
                                                     uint32_t InfoSize);
#endif
...
#ifdef WIN32
// Export the signature only; the routine will be linked through GetProcAdddress
typedef uint32_t(STD_CALL *pSTLink_OpenDevice)(TEnumStlinkInterface IfId, uint8_t DevIdxInList, uint8_t bExclusiveAccess, void **pHandle);
#else
EXPORTED_API uint32_t STD_CALL STLink_OpenDevice(TEnumStlinkInterface IfId, uint8_t DevIdxInList, uint8_t bExclusiveAccess, void **pHandle);
#endif
...
#ifdef WIN32
// Export the signature only; the routine will be linked through GetProcAdddress
typedef uint32_t(STD_CALL *pSTLink_SendCommand)(void *pHandle, PDeviceRequest pRequest, uint32_t DwTimeOut);
#else
EXPORTED_API uint32_t STD_CALL STLink_SendCommand(void *pHandle, PDeviceRequest pRequest, uint32_t DwTimeOut);
#endif
...
#ifdef WIN32
// Export the signature only; the routine will be linked through GetProcAdddress
typedef uint32_t(STD_CALL *pSTLink_ReenumerateTcp)(TEnumStlinkInterface IfId, uint8_t bClearList, char *pConnectParams,
                                                   char *pServerCmdLineParams);
#else
EXPORTED_API uint32_t STD_CALL STLink_ReenumerateTcp(TEnumStlinkInterface IfId, uint8_t bClearList, char *pConnectParams,
                                                     char *pServerCmdLineParams);
#endif
...
#ifdef WIN32
// Export the signature only; the routine will be linked through GetProcAdddress
typedef uint32_t(STD_CALL *pSTLink_OpenDeviceTcp)(TEnumStlinkInterface IfId, uint32_t DevInfoUsbId, uint8_t bExclusiveAccess);
#else
EXPORTED_API uint32_t STD_CALL STLink_OpenDeviceTcp(TEnumStlinkInterface IfId, uint32_t DevInfoUsbId, uint8_t bExclusiveAccess);
#endif
...
#ifdef WIN32
// Export the signature only; the routine will be linked through GetProcAdddress
typedef uint32_t(STD_CALL *pSTLink_SendCommandTcp)(uint32_t StLinkUsbId, PDeviceRequest pRequest, uint32_t DwTimeOut);
#else
EXPORTED_API uint32_t STD_CALL STLink_SendCommandTcp(uint32_t StLinkUsbId, PDeviceRequest pRequest, uint32_t DwTimeOut);
#endif
