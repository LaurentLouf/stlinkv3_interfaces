 ...
 * StlinkDevice::SetOpenModeExclusive() is used before opening the device to choose the opening
 * mode.
 *
 * @param[in]  StlinkInstId  Instance ID in the list of enumerated STLink devices.
 *                           from 0 to *pNumDevices-1 (value returned by
 ...
 * @retval #BRG_OLD_FIRMWARE_WARNING Warning that current Bridge Firmware version is not the last
 * one
...
Brg_StatusT Brg::OpenStlink(int StlinkInstId) {
    ...
    if (brgStatus == BRG_NO_ERR) {
        ...
        if (IsOldBrgFwVersion() == true) {
            LogTrace(
                "The detected STLink firmware BRIDGE version (V%d.B%d) is compatible with PC "
                "software but is not the most recent one",
...
/**
 * @ingroup DEVICE
 * @brief Same as Brg::OpenStlink(int StlinkInstId) but opening routine based on the serial number
 * of the STLink. \n To be used in case StlinkInstId is no more reliable (devices have been
 * reenumerated between STLink instance selection and opening) or to choose a specific STLink by its
 * serial number.
 *
 * @param[in]  pSerialNumber  STLink serial number (ASCII) (can be retrieved with
 * STLinkInterface::GetDeviceInfo2()).
 * @param[in]  bStrict 	 Used if STLink with required serial number is not found, but one other
 * STLink is found: choose if we open or not the other STLink. \n If bStrict is true, the command
 * will return #BRG_STLINK_SN_NOT_FOUND if the given SerialNumber is not found.\n If bStrict is
 * false and the given serial number is not found and one (and only one) STLink is found, the
 * command will attempt to open the STLink even if the serial number does not match.
 * @warning #BRG_OLD_FIRMWARE_WARNING is not a fatal error.
 * @return Brg::ConvSTLinkIfToBrgStatus(STLinkInterface::EnumDevices()) errors
...
/**
 * @ingroup DEVICE
 * @brief Close STLink USB communication with the device instance that was opened by
 * Brg::OpenStlink().
 *
 * @retval #BRG_NO_ERR If no error
 */
...
/**
 * @ingroup DEVICE
 * @brief This routine closes given or all Bridge communication(s), need to call again Init function
 * after if new bridge communication is required.
...
Brg_StatusT Brg::CloseBridge(uint8_t BrgCom) {
    ...
    if ((BrgCom != COM_SPI) && (BrgCom != COM_I2C) && (BrgCom != COM_CAN) && (BrgCom != COM_GPIO) && (BrgCom != COM_UNDEF_ALL)) {
 ...
 */
Brg_StatusT Brg::SendRequestAndAnalyzeStatus(STLink_DeviceRequestT *pDevReq, const uint16_t *pStatus, const uint16_t UsbTimeoutMs) {
    ...
    if (brgStat == BRG_TARGET_CMD_ERR) {
        ...
        LogTrace(
            "BRIDGE Error (0x%hx) after BRIDGE cmd %02hX %02hX %02hX %02hX %02hX %02hX %02hX %02hX "
            "%02hX %02hX",
            // printf("Error (0x%hx) after target cmd %02hX %02hX %02hX %02hX %02hX %02hX %02hX
            // %02hX %02hX %02hX",
            (unsigned short)*pStatus, (unsigned short)pDevReq->CDBByte[0], (unsigned short)pDevReq->CDBByte[1],
            (unsigned short)pDevReq->CDBByte[2], (unsigned short)pDevReq->CDBByte[3], (unsigned short)pDevReq->CDBByte[4],
            (unsigned short)pDevReq->CDBByte[5], (unsigned short)pDevReq->CDBByte[6], (unsigned short)pDevReq->CDBByte[7],
            (unsigned short)pDevReq->CDBByte[8], (unsigned short)pDevReq->CDBByte[9]);
    }
    ...
    if (pStatus != NULL) {
        if (*pStatus != STLINK_BRIDGE_OK) {
            ...
            if (*pStatus == STLINK_BRIDGE_ABORT_TRANS) {
                LogTrace(
                    "BRIDGE Incorrect command order in partial (I2C) transaction, current "
                    "transaction aborted");
...
/**
 * @ingroup BRIDGE
 * @brief This routine gets some current frequencies, useful to choose bridge initialization
 * parameters.
 ...
 * Call Brg::GetSPIbaudratePrescal() to get baudrate parameter according to required SPI output
 * frequency.
...
/**
 * @ingroup SPI
 * @brief This routine gets the SPI baudrate prescaler needed for Brg::InitSPI() according to
 * required SPI CLK.
...
/**
 * @ingroup SPI
 * @brief This routine allows to manage SPI NSS (nCS) pin when SPI has been initialized with
 * #SPI_NSS_SOFT.
...
/**
 * @ingroup SPI
 * @brief This routine allows to receive bytes on the SPI interface, in the mode initialized by
...
/**
 * @ingroup SPI
 * @brief This routine allows to transmit bytes on the SPI interface, in the mode initialized by
 * Brg::InitSPI().
 ...
 * @param[out] pSizeWritten If not NULL and in case of error, pSizeWritten returns the number of
 * bytes transmitted before the error.
 *
 * @retval #BRG_NO_STLINK If Brg::OpenStlink() not called before
 * @retval #BRG_COM_INIT_NOT_DONE If SPI is not initialized
 ...
 * Refer to STLink firmware upgrade release note (stsw-link007) for supported init configurations.\n
 * Call Brg::GetI2cTiming() to get timing parameter according to required I2C speed and time
...
Brg_StatusT Brg::InitI2C(const Brg_I2cInitT *pInitParams) {
    ...
    // Filters : analog and digital filters config : Bit3-0: DNF (<=15)  Bit7: analog filter 0 OFF,
    // 1 ON
    //  Bit6-4: 0 (reserved)
...
/**
 * @ingroup I2C
 * @brief This routines can be used to get timing parameter (pTimingReg) required by
 * Brg::InitI2C().\n It set timing parameter according to required I2C speed, mode, rise/fall time
 ...
 * @retval #BRG_NO_ERR If no error
 */
Brg_StatusT Brg::GetI2cTiming(I2cModeT I2CSpeedMode, int SpeedFrequency, int DNFn, int RiseTime, int FallTime, bool bAF,
                              uint32_t *pTimingReg) {
    ...
    *pTimingReg = 0;  // Default if error
    ...
    if (brgStatus == BRG_NO_ERR) {
        ...
        brgStatus = CalculateI2cTimingReg(I2CSpeedMode, SpeedFrequency, clockSource, DNFn, RiseTime, FallTime, bAF, pTimingReg);
    }
 ...
 */
Brg_StatusT Brg::CalculateI2cTimingReg(I2cModeT I2CSpeedMode, int SpeedFrequency, double ClockSource, int DNFn, int RiseTime, int FallTime,
                                       bool bAF, uint32_t *pTimingReg) {
#define MAX_I2C_MODEL_NB 40
    ...
    tmpMinSDADEL = (double)((double)fallTimeCalc - (50 / (double)pow((double)10, 9)) - (double)((DNFn + 3) * clkPeriodI2C));
    tmpMaxSDADEL =
        (double)((double)pTHdDatMax[idx] - riseTimeCalc - (1 * (260 / (double)pow((double)10, 9))) - (double)((DNFn + 4) * clkPeriodI2C));
    if (tmpMaxSDADEL < 0) {
        tmpMaxSDADEL = 0;
    }
    if (tmpMinSDADEL < 0) {
        ...
        for (int i3 = nbValid - 1; i3 >= 0; i3--) {
            for (int i1 = 0; i1 < SCLL_LENGTH; i1++) {
                for (int i2 = 0; i2 < SCLH_LENGTH; i2++) {
                    ...
                    if ((speed >= clkMin) && (speed <= clkMax) && (tSclLow >= pTLowMin[idx]) && (tSclHigh >= pTHighMin[idx]) &&
                        (clkPeriodI2C < ((tSclLow - delayFilter) / 4)) && (clkPeriodI2C < tSclHigh)) {
        ...
        for (int i2 = 0; i2 < 16; i2++) {
            for (int i3 = 0; i3 < 16; i3++) {
                for (int j = 0; j < prescSDLDELSCLDELpossCombSize; j++) {
                    if ((pPrescSDLDELSCLDELpossComb[j].resultat == 1) && (pPrescSDLDELSCLDELpossComb[j].prescaler == pPRESCvalid[i3Sel]) &&
                        (pPrescSDLDELSCLDELpossComb[j].SDLDEL == i2) && (pPrescSDLDELSCLDELpossComb[j].SCLDEL == i3) && (x == 0)) {
                        l = pPRESCvalid[i3Sel];
                        sdadel = i2;
                        scldel = i3;
                        x++;
...
/**
 * @ingroup I2C
 * @brief This routine allows to receive bytes on the I2C interface, in the mode initialized by
 * Brg::InitI2C().\n In master mode I2C, the size bytes are received in one I2C transaction
 * ((re)start- addr- data -stop) even if data may be split into several USB packets.
 ...
 * @retval #BRG_COM_CMD_ORDER_ERR If low level I2C function call order is not consitent (Start,
 * Cont, Stop)
 * @retval #BRG_NO_ERR If no error
 */
...
/**
 * @ingroup I2C
 * @brief Same as Brg::ReadI2C(uint8_t *pBuffer, uint16_t Addr, uint16_t SizeInBytes, uint16_t
 * *pSizeRead) except for Addr parameter
 ...
 */
Brg_StatusT Brg::ReadI2C(uint8_t *pBuffer, uint16_t Addr, Brg_I2cAddrModeT AddrMode, uint16_t SizeInBytes, uint16_t *pSizeRead) {
 ...
 */
Brg_StatusT Brg::ReadI2Ccmd(uint8_t *pBuffer, uint16_t Addr, uint16_t SizeInBytes, Brg_I2cRWTransfer RwTransType, uint16_t *pSizeRead,
                            uint32_t *pErrorInfo) {
    STLink_DeviceRequestT *pRq;
    Brg_StatusT brgStat;
...
/**
 * @ingroup I2C
 * @brief This low level routine allows to split I2C transaction and to receive bytes on the I2C
 * interface, in the mode initialized by Brg::InitI2C().\n For a full I2C transaction use
 * Brg::ReadI2C() instead.\n Master mode I2C transaction is splitted: only (re)start, Addr are sent
 * and data are received (no stop sent).
 * @warning After Brg::StartReadI2C() it is mandatory to call Brg::StopReadI2C() (optionally
 * Brg::ContReadI2C()).
 * @param[in]  Addr  I2C slave address used in master mode (default 7bit):
 *             use #I2C_10B_ADDR(Addr) if it is a 10bit address.
 * @param[in]  pBuffer Pointer on data buffer with data to be read.
...
/**
 * @ingroup I2C
 * @brief Same as Brg::StartReadI2C(uint8_t *pBuffer, uint16_t Addr, uint16_t SizeInBytes, uint16_t
 * *pSizeRead) except for Addr parameter
 * @param[in]  Addr Is the I2C slave address used in master mode.
 * @param[in]  AddrMode Indicate if it is a 10bit or 7bit address.
 ...
 */
Brg_StatusT Brg::StartReadI2C(uint8_t *pBuffer, uint16_t Addr, Brg_I2cAddrModeT AddrMode, uint16_t SizeInBytes, uint16_t *pSizeRead) {
    uint16_t slaveAddr = Addr;  // default bit15=0 7b
    if (AddrMode == I2C_ADDR_10BIT) {
        slaveAddr = I2C_10B_ADDR(Addr);  // set bit15 to 1 for 10b
    }
 ...
 * @brief This low level routine allows to split I2C transaction and to receive bytes on the I2C
 * interface, in the mode initialized by Brg::InitI2C().\n For a full I2C transaction use
 * Brg::ReadI2C() instead.\n Master mode I2C transaction is splitted: only data are received (no
 * start-address or stop sent).
 ...
 * @brief This low level routine allows to split I2C transaction and to receive bytes on the I2C
 * interface, in the mode initialized by Brg::InitI2C().\n For a full I2C transaction use
 * Brg::ReadI2C() instead.\n Master mode I2C transaction is splitted: only data are received and
 * stop sent (no start-address sent).
 * @warning Before Brg::StopReadI2C() it is mandatory to call Brg::StartReadI2C() (optionally
 * Brg::ContReadI2C()).
 * @param[in]  pBuffer Pointer on data buffer with data to be read.
 * @param[in]  SizeInBytes Data size to be read in bytes (min 1, max data buffer size)
 * @param[out] pSizeRead If not NULL and in case of error, pSizeRead returns the number of bytes
 ...
 *
 * @retval #BRG_CMD_BUSY If I2C transaction is not completed, use Brg:GetLastReadWriteStatus() until
 * no more BUSY
 * @retval #BRG_NO_STLINK If Brg::OpenStlink() not called before
 * @retval #BRG_COM_INIT_NOT_DONE If I2C is not initialized
 * @retval #BRG_I2C_ERR In case of I2C error
...
/**
 * @ingroup I2C
 * @brief Same as Brg::ReadNoWaitI2C(uint8_t *pBuffer, uint16_t Addr, uint16_t SizeInBytes, uint16_t
 * *pSizeRead) except for Addr parameter
 * @param[in]  Addr Is the I2C slave address used in master mode.
 * @param[in]  AddrMode Indicate if it is a 10bit or 7bit address.
 ...
 */
Brg_StatusT Brg::ReadNoWaitI2C(uint16_t Addr, Brg_I2cAddrModeT AddrMode, uint16_t SizeInBytes, uint16_t *pSizeRead, uint16_t CmdTimeoutMs) {
    uint16_t slaveAddr = Addr;  // Default bit15=0 7b
    if (AddrMode == I2C_ADDR_10BIT) {
        slaveAddr = I2C_10B_ADDR(Addr);  // Set bit15 to 1 for 10b
    }
...
/**
 * @ingroup I2C
 * @brief This routine allows to get the data read on I2C with Brg::ReadNoWaitI2C() once status is
 * no more BUSY. Status is retrieved using Brg::GetLastReadWriteStatus() (#BRG_CMD_BUSY is answered
 * in BUSY state).
 * @warning If other commands than STLINK_BRIDGE_GET_RWCMD_STATUS are sent to the STLink firmware
 *  while it is in BUSY state this can break the USB communication.
 *
 * @param[in]  pBuffer Pointer on data buffer with data to be read.
 * @param[in]  SizeInBytes Data size to be read in bytes must be same as the one used in
 * Brg::ReadNoWaitI2C() (min 1, max data buffer size).
 *
 * @retval #BRG_NO_STLINK If Brg::OpenStlink() not called before
 * @retval #BRG_COM_INIT_NOT_DONE If I2C is not initialized
 * @retval #BRG_I2C_ERR In case of I2C error
...
/**
 * @ingroup I2C
 * @brief This routine allows to transmit bytes on the I2C interface, in the mode initialized by
 * Brg::InitI2C().\n In master mode I2C the size bytes are transmitted in one I2C transaction
 * ((re)start- addr- data -stop) even if data may be split into several USB packets.
 * @param[in]  Addr  I2C slave address used in master mode (default 7bit):
 *             use #I2C_10B_ADDR(Addr) if it is a 10bit address.
 * @param[in]  pBuffer Pointer on data buffer with data to be read.
 ...
 */
Brg_StatusT Brg::WriteI2C(const uint8_t *pBuffer, uint16_t Addr, Brg_I2cAddrModeT AddrMode, uint16_t SizeInBytes, uint16_t *pSizeWritten) {
    uint16_t slaveAddr = Addr;  // default bit15=0 7b
    if (AddrMode == I2C_ADDR_10BIT) {
        slaveAddr = I2C_10B_ADDR(Addr);  // set bit15 to 1 for 10b
    }
 ...
 * - if not NULL, pErrorInfo currently unused
 */
Brg_StatusT Brg::WriteI2Ccmd(const uint8_t *pBuffer, uint16_t Addr, uint16_t Size, Brg_I2cRWTransfer RwTransType, uint16_t *pSizeWritten,
                             uint32_t *pErrorInfo) {
    STLink_DeviceRequestT *pRq;
    Brg_StatusT brgStat;
...
/**
 * @ingroup I2C
 * @brief This low level routine allows to split I2C transaction and to send bytes on the I2C
 * interface, in the mode initialized by Brg::InitI2C().\n For a full I2C transaction use
 * Brg::WriteI2C() instead.\n Master mode I2C transaction is splitted: only (re)start- addr- data
 * are sent (no stop).
 * @param[in]  Addr  I2C slave address used in master mode (default 7bit):
 *             use #I2C_10B_ADDR(Addr) if it is a 10bit address.
 * @param[in]  pBuffer Pointer on data buffer with data to be read.
...
/**
 * @ingroup I2C
 * @brief Same as Brg::StartWriteI2C(uint8_t *pBuffer, uint16_t Addr, uint16_t SizeInBytes, uint16_t
 * *pSizeWritten) except for Addr parameter
 * @param[in]  Addr Is the I2C slave address used in master mode.
 * @param[in]  AddrMode Indicate if it is a 10bit or 7bit address.
 ...
 */
Brg_StatusT Brg::StartWriteI2C(const uint8_t *pBuffer, uint16_t Addr, Brg_I2cAddrModeT AddrMode, uint16_t SizeInBytes,
 ...
 * @brief This low level routine allows to split I2C transaction and to send bytes on the I2C
 * interface, in the mode initialized by Brg::InitI2C().\n For a full I2C transaction use
 * Brg::WriteI2C() instead.\n Master mode I2C transaction is splitted: only data and stop are sent
 * (no start-address).
 * @warning Before Brg::StopWriteI2C() it is mandatory to call Brg::StartWriteI2C() (optionally
 * Brg::ContWriteI2C()).
 * @param[in]  pBuffer Pointer on data buffer with data to be read.
 * @param[in]  SizeInBytes Data size to be read in bytes (min 1, max data buffer size).
 * @param[out] pSizeWritten If not NULL and in case of error, pSizeWritten returns the number of
...
Brg_StatusT Brg::InitCAN(const Brg_CanInitT *pInitParams, Brg_InitTypeT InitType) {
    ...
    // Bit Time segment2 BS2 (PHASE_SEG2) and SJW (Synchronisation Jump Width) CAN_1_time_quantum =
    // 0 Bit 2-0:  segment2 (BS2) Bit 4-3: SJW
...
/**
 * @ingroup CAN
 * @brief This routine gets the CAN  prescaler needed for Brg::InitCAN() according to required CAN
 * bitrate.\n Refer to STLink firmware upgrade release note (stsw-link007) for supported init
 * configurations.
 ...
 * @retval #BRG_NO_ERR If no error
 */
Brg_StatusT Brg::GetCANbaudratePrescal(const Brg_CanBitTimeConfT *pBitTimeConf, uint32_t ReqBaudrate, uint32_t *pPrescal,
    ...
    if (brgStat == BRG_NO_ERR) {
        /* Bit timing
          ...
          the sample point by synchronizing on the start-bit edge and resynchronizing on the
          following edges. Its operation may be explained simply by splitting nominal bit time into
          ...
          time quanta but may also be automatically shortened to compensate for negative phase
          drifts. The resynchronization Jump Width (SJW) defines an upper bound to the amount of
          lengthening or shortening of the bit segments. It is programmable between 1 and 4 time
          quanta Tq = time quanta BaudRate = 1/NominalBitTime
          <--------------- Nominal bit Time --------------------------------->
...
Brg_StatusT Brg::StartMsgReceptionCAN(void) {
    ...
    if (brgStat != BRG_NO_ERR) {
        LogTrace("CAN Error (%d) in StartMsgReceptionCAN (firmware msg format: %d, host format: %d)", (int)brgStat, (int)answer[2],
                 (int)CAN_MSG_FORMAT_V1);
    }
 ...
 * @warning     If MsgNb > Brg::GetRxMsgNbCAN(), firmware will return status error (2bytes) instead
 * of requested messages this may break the USB com.
 ...
 *                If BufSizeInBytes is smaller than the sum of all DLC only BufSizeInBytes are
 * copied into pBuffer.
 ...
 * @retval #BRG_NO_ERR If no error
 */
Brg_StatusT Brg::GetRxMsgCAN(Brg_CanRxMsgT *pCanMsg, uint16_t MsgNb, uint8_t *pBuffer, uint16_t BufSizeInBytes,
                             uint16_t *pDataSizeInBytes) {
    STLink_DeviceRequestT *pRq;
    Brg_StatusT brgStat;
    uint8_t *pAnswer;
    uint8_t *pReadCanMsg;
    ...
    *pDataSizeInBytes = 0;  // Default
    ...
    if (brgStat == BRG_NO_ERR) {
        ...
        for (int j = 0; j < MsgNb; j++) {
            ...
            pCanMsg[j].ID = (uint32_t)pReadCanMsg[0] | (((uint32_t)pReadCanMsg[1]) << 8) | (((uint32_t)pReadCanMsg[2]) << 16) |
            ...
            if (overrunErr != 0) {
                ...
                if (brgStat == BRG_NO_ERR) {
                    ...
                    firstErrMsgNb = j;
                    LogTrace("CAN Overrun Error in GetRxMsgCAN (first error %d at %d/%d msg)", (int)overrunErr, (int)firstErrMsgNb,
                             (int)MsgNb);
                }
            ...
            if ((pReadCanMsg[4] & 0x2) == 0) {  // byte4 bit1 RTR
                ...
                if (buffDataSize >= pCanMsg[j].DLC) {
                    ...
                    if (brgStat == BRG_NO_ERR) {
                        brgStat = BRG_OVERRUN_ERR;
                        LogTrace(
                            "CAN Data Error in GetRxMsgCAN: BufSizeInBytes too small (error at "
                            "%d/%d msg)",
                            (int)j, (int)MsgNb);
                    }
                }
    ...
    if (brgStat != BRG_NO_ERR) {
        LogTrace("CAN Error (%d) in GetRxMsgCAN (max %d bytes, %d msg)", (int)brgStat, (int)BufSizeInBytes, (int)MsgNb);
    }
#ifdef USING_TRACELOG
    else {
...
Brg_StatusT Brg::WriteMsgCAN(const Brg_CanTxMsgT *pCanMsg, const uint8_t *pBuffer, uint8_t SizeInBytes) {
    ...
    if (brgStat == BRG_NO_ERR) {  // pSizeWritten not useful for CAN, pErrorInfo currently unused
...
// -------------------------------- GPIO ----------------------------------- //
/*
 * private: return the gpio configuration field of STLINK_BRIDGE_INIT_GPIO according to init
 * parameter
 */
...
Brg_StatusT Brg::InitGPIO(const Brg_GpioInitT *pInitParams) {
    ...
    if (((pInitParams->ConfigNb != 1) && (pInitParams->ConfigNb != BRG_GPIO_MAX_NB)) || ((pInitParams->GpioMask & BRG_GPIO_ALL) == 0)) {
        return BRG_PARAM_ERR;
    }
...
Brg_StatusT Brg::ReadGPIO(uint8_t GpioMask, Brg_GpioValT *pGpioVal, uint8_t *pGpioErrorMask) {
    ...
    *pGpioErrorMask = answer[2];
    ...
    // Answer byte3 GPIO read value (0 or 1), if GPIO is present in the mask, retrieve the read
    // value
...
Brg_StatusT Brg::SetResetGPIO(uint8_t GpioMask, const Brg_GpioValT *pGpioVal, uint8_t *pGpioErrorMask) {
    ...
    // GPIO mask
    pRq->CDBByte[2] = GpioMask;
    // GPIO set reset mask (1 if  set, 0 if reset), if GPIO is present in the mask set the value to
    // write.
    for (int i = 0; i < BRG_GPIO_MAX_NB; i++) {
        if ((GpioMask & (1 << i)) != 0) {
