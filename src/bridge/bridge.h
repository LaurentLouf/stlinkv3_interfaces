...
typedef enum {
    ...
    BRG_CMD_BUSY,                ///< Command busy: only Brg::GetLastReadWriteStatus() command allowed in that
                                 ///< case
...
typedef enum {
    SPI_CPHA_1EDGE = 0,  ///< SCK 1st edge: data captured on 1st CLK edge (rising if CPOL low or
                         ///< falling if CPOL high),
...
typedef struct {
    ...
    ///< Note for smaller delay: NSS mode hardware pulse insert one SPI CLK cycle (when NSS is
    ///< pulsed)\n Note for larger delay: call N time Read/Write function transmitting 1 byte
    ///< instead of 1 time
...
/// AddressingMode : for slave address for master and Own address for slave mode, see also \ref
/// LIMITATION
typedef enum {
...
typedef struct {
    uint32_t TimingReg;              ///< Call Brg::GetI2cTiming() to get TimingReg parameter according to
                                     ///< required I2C speed and time configuration parameters.
...
///
///	In Silent mode, the CAN Bridge is able to receive valid data frames and valid remote frames,
/// but 	it sends only recessive bits on the CAN bus and it cannot start a transmission. If the CAN
...
typedef struct {
    ...
    uint8_t DLC;         ///< Data Length Code, max 8. Number of requested data for RTR, unused for data
                         ///< Frame. (for data frame Size parameter of Brg::WriteMsgCAN() is used as DLC)
} Brg_CanTxMsgT;
...
///     <table>
///     <tr><th>Scale              <th>Mode    <th>Id[0] <th>Mask[0] <th>Id[1] <th>Mask[1] <th>Id[2]
///     <th>Id[3] <tr><td rowspan="2">32bit  <td>ID_MASK <td>x     <td>x       <td>-     <td>- <td>-
///     <td>- <tr>                       <td>ID_LIST <td>x     <td>-       <td>x     <td>- <td>-
///     <td>- <tr><td rowspan="2">16bit  <td>ID_MASK <td>x     <td>x       <td>x     <td>x <td>-
///     <td>- <tr>                       <td>ID_LIST <td>x     <td>-       <td>x     <td>- <td>x
///     <td>x
///     </table>
// -------------------------------------------------------------------
...
class Brg : public StlinkDevice {
   public:
    ...
    Brg_StatusT GetI2cTiming(I2cModeT I2CSpeedMode, int SpeedFrequency, int DNFn, int RiseTime, int FallTime, bool bAF,
                             uint32_t *pTimingReg);
    Brg_StatusT ReadI2C(uint8_t *pBuffer, uint16_t Addr, uint16_t SizeInBytes, uint16_t *pSizeRead);
    Brg_StatusT ReadI2C(uint8_t *pBuffer, uint16_t Addr, Brg_I2cAddrModeT AddrMode, uint16_t SizeInBytes, uint16_t *pSizeRead);
    Brg_StatusT StartReadI2C(uint8_t *pBuffer, uint16_t Addr, uint16_t SizeInBytes, uint16_t *pSizeRead);
    Brg_StatusT StartReadI2C(uint8_t *pBuffer, uint16_t Addr, Brg_I2cAddrModeT AddrMode, uint16_t SizeInBytes, uint16_t *pSizeRead);
    ...
    Brg_StatusT ReadNoWaitI2C(uint16_t Addr, uint16_t SizeInBytes, uint16_t *pSizeRead, uint16_t CmdTimeoutMs);
    Brg_StatusT ReadNoWaitI2C(uint16_t Addr, Brg_I2cAddrModeT AddrMode, uint16_t SizeInBytes, uint16_t *pSizeRead, uint16_t CmdTimeoutMs);
    ...
    Brg_StatusT WriteI2C(const uint8_t *pBuffer, uint16_t Addr, Brg_I2cAddrModeT AddrMode, uint16_t SizeInBytes, uint16_t *pSizeWritten);
    Brg_StatusT StartWriteI2C(const uint8_t *pBuffer, uint16_t Addr, uint16_t SizeInBytes, uint16_t *pSizeWritten);
    Brg_StatusT StartWriteI2C(const uint8_t *pBuffer, uint16_t Addr, Brg_I2cAddrModeT AddrMode, uint16_t SizeInBytes,
    ...
    Brg_StatusT GetCANbaudratePrescal(const Brg_CanBitTimeConfT *pBitTimeConf, uint32_t ReqBaudrate, uint32_t *pPrescal,
    ...
    Brg_StatusT GetRxMsgCAN(Brg_CanRxMsgT *pCanMsg, uint16_t MsgNb, uint8_t *pBuffer, uint16_t BufSizeInBytes, uint16_t *pDataSizeInBytes);
   ...
   private:
    ...
    Brg_StatusT SendRequestAndAnalyzeStatus(STLink_DeviceRequestT *pDevReq, const uint16_t *pStatus, const uint16_t UsbTimeoutMs = 0);

    Brg_StatusT WriteI2Ccmd(const uint8_t *pBuffer, uint16_t Addr, uint16_t Size, Brg_I2cRWTransfer RwTransType, uint16_t *pSizeWritten,
                            uint32_t *pErrorInfo);
    Brg_StatusT ReadI2Ccmd(uint8_t *pBuffer, uint16_t Addr, uint16_t SizeInBytes, Brg_I2cRWTransfer RwTransType, uint16_t *pSizeRead,
                           uint32_t *pErrorInfo);
    ...
    Brg_StatusT CalculateI2cTimingReg(I2cModeT I2CSpeedMode, int SpeedFrequency, double ClockSource, int DNFn, int RiseTime, int FallTime,
